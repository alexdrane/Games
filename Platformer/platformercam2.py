...
def start():
  global FPS,fpsClock,DISPLAY,you,stone,char,bounce,bounce2,level,cannon_png,back,warp,SCREEN
  level = 0
  pygame.init()
  ...
  SCREEN = pygame.display.set_mode((1200,700),FULLSCREEN)
...
def run():
  ...
  while True:
    ...
    for floor in blocks[level]:
      ...
      if floor[4] == 3 and (clock+floor[7]) % floor[5] == 0:
        if floor[6]:
          ...
          floor[6] = True
          if floor[0]<you[0] and you[0]+15<floor[0]+floor[2] and floor[1]<you[1]-15 and you[1]-2<floor[1]+floor[3]:
            velocity_y = 0
            velocity_x = 0
            pygame.mixer.music.load("die.mp3")
            pygame.mixer.music.play()
    ...
    SCREEN.blit(pygame.transform.scale(pygame.transform.chop(DISPLAY,(600,350,you[0]-300,you[1]-175)),(1200,700)),(0,0))
    pygame.display.update()
    fpsClock.tick(FPS)
    clock+=1
